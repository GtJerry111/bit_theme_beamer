...
% ----------------
\usetheme[
	ColorDisplay=BSblue, % JXred ⚙️ | BSblue | Custom → 主题色显示设置
	% BlockDisplay=colorful, % colorful ⚙️ | followtheme | allgrey → 区块颜色显示设置
	CodeTheme=minted, % listing ⚙️ | minted | minted2 → 代码高亮显示设置
...
\begin{frame}{Info.}
	...
	\faGithub\enspace{\color{bitb}\url{https://github.com/FvNCCR228/SCU-Beamer-Theme}}
...
\begin{frame}[fragile]{A Stared Source Code Block}
	...
	\end{scucode*}
	\begin{bitcode}{Another Stared Theorem Block.}*[pythoncode]{python}
		for i in range(1,5):
			for j in range(1,5):
				for k in range(1,5):
					if( i != k ) and (i != j) and (j != k):
...
\begin{frame}[fragile]{Highlight Line}
	\begin{bitcode}{Highlight Line.}[Highlight1]{c}[highlightlines={2,5}]
		#include <iostream>
		int main()
		{
			std::cout << "Hello World！" << std::endl;
	...
	\end{bitcode}
	\begin{bitcode}{Highlight Line.}[pythoncode]{python}[highlightlines={2-3,5}]
		for i in range(1,5):
			for j in range(1,5):
				for k in range(1,5):
					if( i != k ) and (i != j) and (j != k):
...
\begin{frame}[fragile]{Overlay \& Label}{Escapeinline}
	...
	\end{bitcode}
	\begin{bitcode}{Comment}[Escapeinline2]{python}[highlightlines={4}]'@@'
		for i in range(1,5):
			for j in range(1,5): 
				for k in range(1,5):
